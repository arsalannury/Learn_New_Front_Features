import { Sentence } from "./mock/sentence";
import { useRandomText } from "./store/counter";

function App() {
  const { randomText, setRandomText } = useRandomText();
  const generateRandomText: string = Sentence[Math.floor(Math.random() * 5)];
  return (
    <>
      <div className="absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] flex items-center flex-col justify-around">
        <p className="mb-10 font-bold text-xl">{randomText}</p>
        <div>
          <button
